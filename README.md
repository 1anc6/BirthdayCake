# READ-ME

# 👋 Whasupp Chooms!! I'm Lance.

### 🎮 Aspiring Game Developer & Tech Enthusiast

I'm a passionate BTech Computer Science Engineering student with a keen interest in programming and creating video game-based projects.
I enjoy exploring new technologies and bringing creative ideas to life through code.

---

## 💻 Languages & Tools
- **C**
- **C++**
- **Python**

## 🚀 Featured Projects

- ### 🕹️ Tetris
  Developed a classic Tetris game in Python as part of my college term work, focusing on game logic, user interaction, and interface design.

- ### ❌⭕ Tic Tac Toe
  Built a basic Tic Tac Toe game using Python, honing my skills in algorithm design and user experience.


---

## 🎯 What I'm Up To
- Expanding my skills in game development
- Building cool and fun projects as I learn

## 🎲 Hobbies & Fun Facts
- Playing video games
- Watching movies

---

<!-- GitHub Stats Badge (optional) -->
<!-- ![1anc6's GitHub stats](https://github-readme-stats.vercel.app/api?username=1anc6&show_icons=true&theme=radical) -->

See you around chooms !
